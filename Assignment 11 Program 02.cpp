/* This program utilizes structures for information
on four players (name, number, points) and asks for
the info, displays the info, and then sums all the
points and names the person with the most points.*/
#include <iostream>
#include <iomanip>
#include <string>
using namespace std;
...
int main()
{
	...
	for (index = 0; index < NUM_PLAYERS; index++)
	{
		total = getTotalPoints(team, NUM_PLAYERS);
	}
...
void showHighest(Player p[], int size)
{
	...
	{
		if (p[index].points > highest)
		{
			highest = p[index].points;
			element = index;
		}
	}
	cout << "The player who scored the most points is: " << p[element].name << endl;
}
